...
| 协议               | 搭配加密      | 搭配混淆  |
|------------------|-----------|-------|
| auth_aes128_md5  | 非 AEAD 加密 | `plain` / `http_simple` / `http_post` / `tls1.2_ticket_auth` / `tls1.2_ticket_fastauth` |
| auth_aes128_sha1 | 非 AEAD 加密 | 同上 |
| auth_chain_a     | 非 AEAD 加密 | 同上 |
| auth_chain_b     | 非 AEAD 加密 | 同上 |
| auth_chain_c     | 非 AEAD 加密 | 同上 |
| auth_chain_d     | 非 AEAD 加密 | 同上 |
| auth_chain_e     | 非 AEAD 加密 | 同上 |
| auth_chain_f     | 非 AEAD 加密 | 同上 |

协议式单端口靠 `auth_chain` / `auth_aes128` 协议头识别用户。使用 HTTP 或 ticket obfs 时，heki 会先完成并剥离混淆层，再进入协议认证；服务端响应会重新封装成相同 obfs。`simple_obfs_http` / `simple_obfs_tls` 属于上面的混淆式单端口和 SS 客户端路径，不要填入协议式单端口。

协议式单端口说明：

| 参数名             | 当前状态 |
|-----------------|------|
| `ssr_cid_limit` | 旧版 SSR 单端口参数，现在通常不用配置。限制同一用户可使用的 IP/设备数量时，请看面板下发的 `device_limit`；面板没有下发时，再使用主配置里的 `user_conn_limit` |

...
!> 如果你希望订阅客户端稳定走 `VMessAEAD`，建议在订阅输出里直接删除 `alterId` 字段，不要仅写 `alterId=0`。
!> `Clash/Mihomo` 等客户端在保留 `alterId` 字段时，即使值为 `0`，也可能继续走 legacy VMess。只要明确保留 `alterId>0`，heki 会自动锁到 `VMessMD5-only`，避免 AEAD/MD5 混跑。

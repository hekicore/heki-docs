...
XBoard 的 Trojan `tls=2`（Reality）也已支持；若走 Reality，则不需要外部证书。
...
- `network_settings.path` / `network_settings.host` 仍分别对应 heki 的 `trojan_h2_path` / `trojan_h2_host`
...
- 部分 XBoard 分支会把 Trojan `server_name` 返回成 `null`；只要 `network_settings.host` 和证书 SNI 正常，heki 仍会按有效 host/SNI 启动

一个常见的 XBoard `protocol_settings` 示例可以写成这样：

```json
{
  "server_name": "trojan-xhttp.example.com",
  "network": "xhttp",
  "tls": 1,
  "network_settings": {
    "path": "/trojan-xhttp",
    "host": "trojan-xhttp.example.com",
    "mode": "auto",
    "extra": {
      "no_sse_header": "true",
      "xmux": {
...
- 证书优先级为 `本地手动证书 > 面板下发证书内容 > 自动申请证书`；如果你已经显式填写 `cert_file/key_file` 或 `trojan_cert_file/key_file`，面板 cert push 不会覆盖它们
...
!> Trojan 走 TLS 时需要证书；若 XBoard 下发的是 `tls=2` Reality，则不需要外部证书。
